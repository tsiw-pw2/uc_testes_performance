/**
 * RF09: Registar resíduos recolhidos — registar recolha numa campanha.
 *
 * Resultado esperado: a recolha é gravada e reflectida no índice de poluição.
 *
 * Pré-requisitos:
...
/** Etiquetas possíveis do peso estimado na UI (por ordem de preferência). */
...
async function aguardarOpcoesResiduos(driver) {
  ...
  await driver.wait(async () => {
    ...
    const residuo = comboboxes[1];
    const disabled = await residuo.getAttribute("disabled");
    const ariaDisabled = await residuo.getAttribute("aria-disabled");
    const className = (await residuo.getAttribute("class")) || "";
    return (
      disabled !== "true" &&
      ariaDisabled !== "true" &&
...
async function verificarActualizacaoIndice(indiceAntes, indiceDepois) {
  ...
  if (indiceDepois.texto === "—" || indiceDepois.texto === "-") {
    throw new Error(
      "Após a recolha, o índice de poluição continua «—»; a recolha pode não ter sido contabilizada.",
    );
  }

  console.log(
    `Índice de poluição actualizado: ${indiceAntes.texto} (${indiceAntes.valor}) → ${indiceDepois.texto} (${indiceDepois.valor}) [${indiceDepois.etiqueta}].`,
  );
}
...
async function main() {
  ...
  try {
    ...
    await executarPasso(driver, 2, "Ler índice de poluição antes da recolha", "indice_antes", async () => {
