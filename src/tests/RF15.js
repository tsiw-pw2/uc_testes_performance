/**
 * RF15: Eliminar campanhas — remover campanha da listagem.
 *
 * Resultado esperado: ao confirmar a eliminação, a campanha desaparece da lista.
 *
 * Pré-requisitos:
...
const PREFIXO_CAMPANHA =
  process.env.CAMPAIGN_TEST_PREFIX || "Campanha eliminar Selenium";
...
const DATA_INICIO =
  process.env.CAMPAIGN_START_DATE || dataDaquiADiasEm2026(21);
const DATA_FIM = process.env.CAMPAIGN_END_DATE || dataDaquiADiasEm2026(21);
...
async function abrirListaCampanhas(driver) {
  ...
  await pausa(driver);
  console.log("Lista de campanhas aberta.");
}
...
async function criarCampanhaTeste(driver) {
  ...
  const criar = await modal.findElement(
    By.xpath(
  ...
  await aguardarCarregamento(driver);
  console.log(`Campanha de teste criada: «${TITULO_CAMPANHA}».`);
}
...
async function campanhaVisivelNaLista(driver, titulo) {
  const linhas = await driver.findElements(By.xpath(xpathLinhaCampanha(titulo)));
  return linhas.length > 0;
}

async function confirmarCampanhaNaLista(driver) {
  await pesquisarCampanha(driver, TITULO_CAMPANHA);
  const visivel = await campanhaVisivelNaLista(driver, TITULO_CAMPANHA);
  if (!visivel) {
    throw new Error(`Campanha «${TITULO_CAMPANHA}» não encontrada na lista.`);
  }
...
async function eliminarCampanhaNaLista(driver) {
  ...
  const botaoEliminar = await linha.findElement(
    By.xpath(
      ".//button[@aria-label='Apagar' or @aria-label='Eliminar' or contains(normalize-space(),'Eliminar')]",
    ),
  );
  await pausa(driver, 450);
  await botaoEliminar.click();
  ...
  const confirmar = await driver.wait(
    until.elementLocated(
      By.xpath(
        "//*[@id='delete-campaign-title']/ancestor::div[@role='dialog']//button[normalize-space()='Apagar Campanha' or normalize-space()='Eliminar Campanha' or normalize-space()='Eliminar']",
      ),
    ),
    10000,
  );
  ...
  await driver.wait(async () => {
    const modais = await driver.findElements(By.id("delete-campaign-title"));
    return modais.length === 0;
  }, 15000);
...
async function confirmarCampanhaRemovida(driver) {
  await pesquisarCampanha(driver, TITULO_CAMPANHA);
  await driver.wait(async () => {
    return !(await campanhaVisivelNaLista(driver, TITULO_CAMPANHA));
  }, 20000);
  console.log("Campanha removida da listagem.");
}
...
async function main() {
  ...
  try {
    ...
    await executarPasso(driver, 2, "Criar campanha de teste", "campanha_criada", async () => {
